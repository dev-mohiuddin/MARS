...
function AccountBalanceTranTable ({ transactions }) {
    const totalIncome = transactions.reduce((acc, transaction) => acc + transaction.credit, 0)
    const totalExpenses = transactions.reduce((acc, transaction) => acc + transaction.debit, 0)
                ...
                <TableBody>
                    {transactions.map((transaction, index) => (
                        <TableRow key={index}>
                            ...
                            <TableCell>{transaction.debit > 0 ? `$${transaction.debit}` : '-'}</TableCell>
                            <TableCell>{transaction.credit > 0 ? `$${transaction.credit}` : '-'}</TableCell>
